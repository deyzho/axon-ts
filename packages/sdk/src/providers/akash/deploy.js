 ...
 */
async function uploadToIpfs(content, ipfsUrl, apiKey) {
    let parsedUrl;
    try {
    ...
    if (parsedUrl.protocol !== 'https:') {
        throw new Error(`AKASH_IPFS_URL must use https:// to protect bundle uploads in transit. Got: "${parsedUrl.protocol}"`);
    }
    const PRIVATE_HOST_RE = /^(localhost|127\.\d+\.\d+\.\d+|10\.\d+\.\d+\.\d+|172\.(1[6-9]|2\d|3[01])\.\d+\.\d+|192\.168\.\d+\.\d+|169\.254\.\d+\.\d+|\[?::1\]?|0\.0\.0\.0)$/i;
    if (PRIVATE_HOST_RE.test(parsedUrl.hostname)) {
        throw new Error(`AKASH_IPFS_URL hostname "${parsedUrl.hostname}" resolves to a private address — blocked to prevent SSRF.`);
    }
...
export function generateAkashSdl(options) {
    const { bundleCid, environment = {}, replicas = 1, maxUaktPerBlock = 10_000, projectName = 'phonix-app', } = options;
...
// ─── CLI helpers ──────────────────────────────────────────────────────────────
async function resolveAkashCli() {
    ...
    throw new Error('Akash CLI not found. Install provider-services:\n' +
        '  https://docs.akash.network/guides/cli/akash-provider-services');
}
async function runAkashCli(args, env) {
...
export async function akashDeploy(options) {
    ...
    if (!mnemonic) {
        throw new Error('AKASH_MNEMONIC is not set. Add it to your .env file.\n' +
            'Run: phonix auth akash  to generate or import one.');
    }
    if (!ipfsUrl) {
        throw new Error('AKASH_IPFS_URL is not set. Akash deployments require IPFS to distribute the bundle.\n' +
    ...
    try {
        ...
        const output = await runAkashCli(['tx', 'deployment', 'create', sdlPath, '--fees', '5000uakt'], env);
...
export async function akashEstimate(config) {
...
export async function akashListDeployments(mnemonic) {
    const env = {};
    const resolvedMnemonic = mnemonic ?? process.env['AKASH_MNEMONIC'] ?? '';
    if (resolvedMnemonic)
        env['AKASH_MNEMONIC'] = resolvedMnemonic;
