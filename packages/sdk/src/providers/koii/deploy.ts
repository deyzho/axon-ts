 ...
 */
function assertSafeUrl(rawUrl: string, label: string): URL {
  let parsed: URL;
  try {
  ...
  if (PRIVATE_RE.test(parsed.hostname)) {
    throw new Error(
      `${label}: requests to private/internal addresses are blocked ("${parsed.hostname}")`
    );
  }
  return parsed;
}
...
async function uploadToIpfs(
  ...
  if (ipfsApiKey) headers['Authorization'] = `Bearer ${ipfsApiKey}`;
  ...
  if (!cid) throw new Error('IPFS response did not contain a CID');
  return cid;
}

// ─── Koii CLI helpers ─────────────────────────────────────────────────────────

async function resolveKoiiCli(): Promise<string> {
  try {
...
function parseNodeIds(output: string): string[] {
  const ids: string[] = [];
  const nodeRegex = /node[^\n]*?:\s*([A-Za-z0-9]{32,44})/gi;
  let match: RegExpExecArray | null;
  while ((match = nodeRegex.exec(output)) !== null) ids.push(match[1]);
  return [...new Set(ids)];
}
...
export async function koiiDeploy(options: KoiiDeployOptions): Promise<Deployment> {
  ...
  if (!ipfsUrl) {
    throw new Error(
      'KOII_IPFS_URL is not set. Add it to your .env file.\n' +
  ...
  try {
    ...
    if (process.env['KOII_NETWORK']) env['KOII_CLUSTER'] = process.env['KOII_NETWORK'];
 ...
 */
export async function koiiEstimate(config: DeploymentConfig): Promise<number> {
 ...
 */
export async function koiiListDeployments(secretKey?: string): Promise<
  Array<{ id: string; processorIds: string[]; status: string }>
> {
  const env: Record<string, string> = {};
  const key = secretKey ?? process.env['KOII_PRIVATE_KEY'] ?? '';
  if (key) env['KOII_PRIVATE_KEY'] = key;
